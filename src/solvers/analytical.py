...
from src.solvers.base import IKSolver, IKSolution
...
class AnalyticalIKSolver(IKSolver):
    ...
    def solve(self, target_pos: np.ndarray, elbow_up: bool = True) -> List[IKSolution]:
        ...
        solutions = [
            IKSolution(np.array([q1, q2, q3]), True)
        ]
        
        # Also return the other branch if requested (but here we just return the preferred one)
        return solutions
